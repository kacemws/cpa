...
void resize2D(int w,int h,Byte *A,int w2,int h2,double *B,int la) {
  int x,y,count = 0, min = 1000, max = 0, pmin,pmax;
  double v[MAX_SIZE];
#pragma omp parallel private(count)
{
	  /* Resize height from A to B: A ( w * h ) -> B ( w * h2 ) */
#pragma omp for private(y,v),schedule(dynamic)
  for (x=0;x<w;x++) {
...
#pragma omp for private(x,v),schedule(dynamic)
  for (y=0;y<h2;y++) {
  ...
  if(count > max){
	...
	if(count > max)
		max = count;
    pmax = omp_get_thread_num();
  }
	
}
printf("\t%d (thread %d) - %d (thread %d ) calls",min,pmin,max,pmax);
}
...
int resizeRGB(int w,int h,Byte *A,int w2,int h2,int la) {
  ...
  printf("\nred plane:"); resize2D(w,h,A,w2,h2,aux,la);   /* red */
  printf("\ngreen plane:"); resize2D(w,h,A+1,w2,h2,aux,la); /* green */
  printf("\nblue plane:"); resize2D(w,h,A+2,w2,h2,aux,la); /* blue */
  free(aux);
  return 0;
}
...
int main(int argc,char *argv[]) {
  ...
  double e=4;
  Byte *A;
  char *input="/labos/asignaturas/ETSINF/cpa/p2/Lenna256.ppm",*output="lan2Dx.ppm",c,*arg;
  //char *prog = argv[0];
